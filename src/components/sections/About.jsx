import Reveal from "../ui/Reveal.jsx";
import { Kicker } from "../ui/SectionHeading.jsx";

const FACTS = [
  { value: "1972", label: "Anul zidirii" },
  { value: "23 apr.", label: "Ziua hramului" },
  { value: "3", label: "Turle cu cruci" },
];

export default function About() {
  return (
    <section id="despre" className="scroll-mt-[70px] px-[34px] py-[clamp(64px,9vw,120px)]">
      <div className="mx-auto grid max-w-[1120px] items-center gap-[clamp(36px,6vw,84px)] md:grid-cols-[1.05fr_.95fr]">
        <Reveal>
          <Kicker className="mb-4">Despre parohie</Kicker>
          <h2 className="mb-[22px] font-heading text-[clamp(32px,4.4vw,52px)] leading-[1.08] font-normal tracking-normal">
            O poartă către liniște, deschisă tuturor
          </h2>
          <div className="mb-[26px] h-px w-14 bg-bronze" />
          <p className="mb-4 text-justify text-[16.5px] leading-[1.85] text-ink-soft">
            Ridicată începând cu anul <strong className="font-semibold">1972</strong> prin
            osteneala și dărnicia credincioșilor din Sântana, biserica noastră poartă hramul
            Sfântului Mare Mucenic Gheorghe, purtătorul de biruință. Zidurile ei albe și turlele
            acoperite cu aramă au vegheat de atunci peste generații de familii, botezuri, cununii
            și rugăciuni.
          </p>
          <p className="text-justify text-[16.5px] leading-[1.85] text-ink-soft">
            Astăzi, ea rămâne o casă a rugăciunii unde ușile sunt deschise pentru fiecare — pentru
            cel care caută pacea, pentru cel îndurerat și pentru cel care voiește să mulțumească.
            Vă întâmpinăm cu bucurie.
          </p>

          <div className="mt-[38px] flex flex-wrap gap-[38px]">
            {FACTS.map((fact, i) => (
              <div key={fact.label} className="contents">
                {i > 0 && <div className="w-px self-stretch bg-ink/14" />}
                <div>
                  <div className="font-heading text-[40px] font-normal text-bronze">
                    {fact.value}
                  </div>
                  <div className="mt-1 text-xs tracking-[.06em] text-stone uppercase">
                    {fact.label}
                  </div>
                </div>
              </div>
            ))}
          </div>
        </Reveal>

        <Reveal as="figure" className="plate m-0 shadow-[0_12px_32px_rgba(45,43,43,.18)]">
          <img
            src="/assets/church-facade.png"
            alt="Fațada bisericii în lumina soarelui"
            className="block h-[clamp(420px,60vh,620px)] w-full object-cover"
          />
        </Reveal>
      </div>
    </section>
  );
}
